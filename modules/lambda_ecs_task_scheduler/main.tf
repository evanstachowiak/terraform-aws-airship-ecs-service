...
resource "aws_cloudwatch_event_rule" "schedule_expressions" {
  ...
  name                = format("job-%.32s", lookup(var.ecs_cron_tasks[count.index], "job_name", "default"))
  description         = "${local.identifier}-${lookup(var.ecs_cron_tasks[count.index], "job_name", "default")}"
  schedule_expression = lookup(var.ecs_cron_tasks[count.index], "schedule_expression", null)
}
...
data "template_file" "task_defs" {
  ...
  vars = {
    ...
    started_by     = format("job-%.32s", lookup(var.ecs_cron_tasks[count.index], "job_name", "default"))
